...
def generate_batch_report_pdf(batch_data):
    ...
    fail_count = sum(1 for r in results if r.get('status') == 'FAILURE')
    error_count = sum(1 for r in results if r.get('status') == 'ERROR')
    
    tracker_rows = ""
    sorted_tracker = sorted(tracker.values(), key=lambda v: (v.get('Placement ID', ''), v.get('Event Type', '')))
    for v in sorted_tracker:
        status_class = "pass" if "Passed" in str(v.get("Result", "")) else ("fail" if "Mismatch" in str(v.get("Result", "")) else "skip")
    ...
    for run_data in sorted_results:
        ...
        for vr_key, vr_val in run_results.items():
            ...
            status_label = "PASS ✅" if overall_success else "FAILURE ⚠️"
            details_html += f"""<div class="spec-section">
                <h3>{placement_label} — {status_label}</h3>
                <p class="suite-ref">Suite: {suite_ref}</p>
                <p class="summary-line">✅ {pass_count} passed | ❌ {fail_count_detail} failed | Total: {pass_count + fail_count_detail} expectations</p>
...
<style>
  ...
  h1 {{ color: #1a73e8; margin-bottom: 4px; font-size: 18px; }}
  h2 {{ font-size: 14px; }}
  .meta {{ color: #666; font-size: 9px; margin-bottom: 12px; }}
  .metrics {{ display: flex; gap: 16px; margin-bottom: 20px; }}
  .metric {{ background: #f8f9fa; border-radius: 8px; padding: 12px 20px; text-align: center; flex: 1; }}
  .metric .num {{ font-size: 22px; font-weight: bold; }}
  .metric.pass .num {{ color: #34a853; }}
  .metric.fail .num {{ color: #ea4335; }}
  .metric.error .num {{ color: #fa7b17; }}
  .metric.total .num {{ color: #1a73e8; }}
  table {{ width: 100%; border-collapse: collapse; margin-bottom: 16px; font-size: 9px; }}
  th {{ background: #1a73e8; color: white; padding: 5px 6px; text-align: left; font-size: 8px; }}
  td {{ padding: 4px 6px; border-bottom: 1px solid #e0e0e0; font-size: 8px; }}
  tr.pass {{ background: #e6f4ea; }}
  tr.fail {{ background: #fce8e6; }}
  tr.skip {{ background: #f1f3f4; }}
  .spec-section {{ margin-bottom: 20px; padding-bottom: 16px; border-bottom: 2px solid #e0e0e0; page-break-inside: avoid; }}
  .spec-section h3 {{ margin: 0 0 4px 0; font-size: 11px; }}
  .summary-line {{ font-size: 9px; margin: 4px 0 8px 0; color: #555; }}
  .suite-ref {{ font-size: 9px; color: #999; margin: 0 0 6px 0; font-style: italic; }}
  .detail-table {{ border: 1px solid #ccc; width: 100%; border-collapse: collapse; }}
  .detail-table th {{ background: #37474f; color: white; font-size: 7px; padding: 3px 4px; text-align: left; }}
  .detail-table td {{ border: 1px solid #ddd; padding: 3px 4px; font-size: 7px; vertical-align: top; word-break: break-word; }}
  .row-pass {{ background: #f1f8e9; }}
  .row-fail {{ background: #fce4ec; }}
  .status-cell {{ text-align: center; width: 24px; }}
  .col-cell {{ font-weight: bold; }}
  .exp-cell {{ font-family: monospace; font-size: 7px; color: #555; }}
  .detail-cell {{ font-size: 7px; line-height: 1.3; }}
  .pass-text {{ color: #2e7d32; }}
  .extra-info {{ color: #666; }}
  .unexpected-val {{ color: #c62828; font-family: monospace; word-break: break-all; }}
...
def bulk_validation_ui():
        ...
        if len(shared["ai_queue"]) > 0:
            ...
            if shared["found_count"] == 0:
                ...
                for run_data in results:
                    ...
                    status_icon = "✅" if run_data['status'] == "SUCCESS" else ("⚠️" if run_data['status'] == "FAILURE" else "❌")
                    
                    with st.expander(f"{status_icon} **{suite}** — {run_data['status']}", expanded=(run_data['status'] != "SUCCESS")):
            ...
            # Save batch to session for sidebar history
            if "current_batch" not in st.session_state:
                ...
                # สร้าง PDF Report
                try:
                    pdf_path = generate_batch_report_pdf(st.session_state.current_batch)
            ...
            if batch.get("pdf_path") and os.path.exists(batch["pdf_path"]):
                st.success(f"📄 PDF Report saved: `{batch['pdf_path']}`")
                with open(batch["pdf_path"], "rb") as f:
                    st.download_button("📥 ดาวน์โหลด PDF Report", f.read(), file_name=os.path.basename(batch["pdf_path"]), mime="application/pdf", use_container_width=True)
    ...
    if st.session_state.stage == "results_done":
        if st.session_state.get("viewing_batch"):
            ...
            st.caption(f"Batch ID: `{batch['id']}` | {batch.get('timestamp', '')}")
            
            # Show tracker table
            if batch.get("tracker"):
                tracker_df = pd.DataFrame(list(batch["tracker"].values()))
                display_cols = [c for c in tracker_df.columns if c != "Docs"]
                st.dataframe(tracker_df[display_cols], use_container_width=True, hide_index=True)
            
            # PDF Download (ถ้ามี)
            if batch.get("pdf_path") and os.path.exists(batch["pdf_path"]):
                with open(batch["pdf_path"], "rb") as f:
                    st.download_button("📥 ดาวน์โหลด PDF Report", f.read(), file_name=os.path.basename(batch["pdf_path"]), mime="application/pdf", key="bh_pdf_dl", use_container_width=True)
            ...
            for run_data in results:
                ...
                with st.expander(f"{status_icon} **{suite}** — {run_data['status']}"):
                    if run_data['status'] == "ERROR":
                        st.error(f"**Error:** {run_data.get('error', 'Unknown')}")
                        continue
